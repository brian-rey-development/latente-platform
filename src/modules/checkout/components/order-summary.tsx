  ...
  return (
    <div className="bg-ink text-surface p-8 border-2 border-ink">
      ...
      <div className="space-y-4 mb-6">
        {items.map((item) => (
          <div key={item._id} className="flex justify-between font-mono text-sm">
      ...
      <div className="flex justify-between items-center pt-4 border-t-2 border-border">
        <span className="font-mono text-xs uppercase tracking-widest text-meta">{t('total')}</span>
        <span className="font-sans font-black text-2xl text-brand">{formatPrice(total)}</span>
      </div>
    </div>
  )
}
